...
```jsx
var str = `Game 1: 2 green, 6 blue, 7 red; 12 green, 6 blue, 3 red; 5 red, 18 green, 4 blue
...
var regex = /Game (\d*):(.*)/gm
var greenRegex = /(\d*) green/gm
var redRegex = /(\d*) red/gm
var blueRegex = /(\d*) blue/gm

var x = [...str.matchAll(regex)]

var data = x.map(m => {
    let x = {};
    let content = m[2];
    console.log(content, [...content.matchAll(blueRegex)])
    let greenMatches = [...content.matchAll(greenRegex)].map(d => Number(d[1]))
    let redMatches = [...content.matchAll(redRegex)].map(d => Number(d[1]))
    let blueMatches = [...content.matchAll(blueRegex)].map(d => Number(d[1]))

    let greenMax = Math.max(...greenMatches)
    let redMax = Math.max(...redMatches)
    let blueMax = Math.max(...blueMatches)

    let greenMin = Math.min(...greenMatches)
    let redMin = Math.min(...redMatches)
    let blueMin = Math.min(...blueMatches)
...
var filteredData = data.filter(da => (da.greenMax <= greenMax) && (da.blueMax <= blueMax) && (da.redMax <= redMax))

console.log(filteredData)

var sum = filteredData.reduce((acc, cur) => acc + cur.id, 0)
console.log(sum) //part`1

var sum = data.reduce((acc, cur) => {
    let g = cur.greenMax || 1
    let r = cur.redMax || 1
    let b = cur.blueMax || 1
    //   console.log(cur.id , g*r*b , {g}, r, b)
    return g * r * b + acc
}, 0)
